...
**问题 1 · 单例持有页面引用（内存泄漏风险）** —— `CameraCapability.getInstance()` → `WeakReference<Delegate>`：WeakReference 只能缓解不能根治；匿名 Delegate 隐式持有 CameraScreen 闭包变量；单例生命周期 > Activity。

**问题 2 · DisposableEffect 时序竞争（delegate 绑定后立即解绑）** —— `CameraScreen 重组 → DisposableEffect.onDispose()`：Compose 重组频繁，onDispose 被过早调用；导航动画期间旧页面先 dispose、新页面后 enter，存在时间窗口。

**问题 3 · Application 级注册僵化（无法动态扩展）** —— `Application.onCreate() → registry.register(capability)`：注册后无法注销、无法热插拔；所有 Capability 常驻内存；单测需清理全局状态。

**问题 4 · SceneManager 与 Compose 生命周期脱节** —— 场景切换与 Composable 生命周期不同步，需要手动对齐。
