...
步骤骨架：
- [ ] batch_read 385:95 子树，记录现有层级（icon 集、label、胶囊指示器、ABSOLUTE 定位）
- [ ] 重造：容器改全宽贴底（x=0, w=393, 底对齐 852）、cornerRadius 仅顶部 0（平底）、删胶囊指示器层、选中色绑 `scheme/primary`、未选中绑 `scheme/onSurfaceVariant`
- [ ] health-check 子树 literal=0 + 钉 Light 导出 + `ardot-light-verify.py`
- [ ] 抽 2 个宿主页（Gallery/Chat）导出快照，像素验证底条形制（横贯 393、顶部 hairline、绿/灰图标）
- [ ] `components.json` 登记 + 组件双模预览入库
...
步骤骨架（逐帧循环）：
- [ ] batch_read 帧结构 → 设计组切分（现卡片语义 → 微信分组语义）
- [ ] 重建容器层级为「组容器 + cell 行 + hairline」；颜色全绑 scheme 变量
- [ ] 每帧：钉 Light 导出 + light-verify + Dark 快照像素抽验（灰底/白组/hairline）
- [ ] 全页 health-check 归零后提交

## T3: Gallery + Chat 页收尾（17 帧残余结构项）

- [ ] 头像 40dp 圆形统一（bubble 旁/侧栏）——若各帧为字面尺寸则逐帧 U；≥2 帧复用考虑入 token `chatBubble.avatarSize`
- [ ] 导航栏形态统一（T2 配方）
- [ ] chatBubble **暗色变体定值**（spec §1 标注「采样校准」：以微信真机 Dark 截图取自方气泡色，如需双值则加 `userBubbleBgDark` 键走完整 token 管线）
- [ ] searchField.backgroundAlpha 0.7 精调（渲染验收后定 1.0 与否）
