...
| `TaskCenter` | `task_center` | 任务中心（2026-09-26，spec US-12~16 提前落地）— 工程师任务跨会话集中管理：进行中/历史分区（历史封顶 50 条）、审批动作（继续/交付/重试/暂不/到此为止）、点击项回 chat 锚定任务卡（Activity 级 `chatTaskAnchor` 一次性请求态，仿 `gallerySearchRequest` 先例）；入口在 Chat 顶栏任务图标（`activeEngineerTaskCount` 角标）与任务卡「查看全部」；VM 经 `AppContainer.createTaskCenterViewModelFactory()` 构建，审批动作复用 Activity 级共享 ChatViewModel 的 `*FromCenter` 入口（prime 补种 + sessionId 显式落库）；2026-09-26 用户任务协议 M1 起改双 Tab（工程师任务 | 后台任务，默认落位=工程师有活跃→工程师、否则后台有活跃→后台、手选固定）：后台 Tab 数据源 = `UserTaskRegistry`（`user_task` 表 + 内存进度合并流），适配器 TAG 扫描/模型下载经组合根 `startUserTaskAdapters()` 注册，顶栏角标 = 工程师活跃 + 用户任务活跃合并计数 |
...
| `usertask/` | `UserTask`（协议模型+5 枚举）, `UserTaskMapping`（状态映射纯函数）, `UserTaskRegistry`（混合注册表）, `UserTaskAdapter`, `TagScanTaskAdapter`, `ModelDownloadTaskAdapter` | 用户任务协议 M1（2026-09-26，spec `docs/superpowers/specs/2026-09-26-user-task-protocol-design.md`）：扫描/下载等 app 级长耗时操作统一生命周期（观察+控制）；元数据落 Room `user_task` 表 + 进度走内存快照，合并流 `tasks`/`activeCount` 直供任务中心后台 Tab；统一动词（START/PAUSE/RESUME/CANCEL/RETRY）按 kind 分发适配器，全链路异常降级不穿透；引擎零侵入，经组合根 `startUserTaskAdapters()` 接缝 |
...
| `backup/` | `TagDataBackup`（model/）, `TagDataBackupRepository`, `BackupTagDataUseCase`, `RestoreTagDataUseCase` | 标签数据备份/恢复（备份模型 v5：标签、人脸聚类、人物关系、记忆事实、编辑配方等本地数据导出导入；v5 增补 mediaTagMetadata 三列 `blurScore`/`exposureScore`/`lastViewedAt`（整理中心 v2 信号），恢复侧 `MediaDao.updateTagMetadataFromBackup` 走 COALESCE——旧备份缺字段（null）不冲掉本机已算值，lastViewedAt 为保护信号不可再生） |
...
| JS 沙箱脚本 | `ChatRunScriptCapability` → `ChatViewModel.onRunScript/onDrawChart/onRenderHtml` → `JsRuntime`（QuickJS） | LLM tool_call（run_gallery_script/draw_chart/render_html）经 CapabilityRegistry（CHAT 场景）落入持久 JsRuntime；`jsEvalMutex` 串行 eval，超时 5s（含 capability.dispatch 写脚本放宽至 180s）；写操作经 CommandRisk 分级 + 用户确认 → `ChatMediaWriteCapability`；render_html 产物（携 display 形态声明）经 `HtmlCardSanitizer` 清洗后落库 HTML_CARD 消息，由 `HtmlCard` WebView 双形态渲染（inline 完全撑开 / fullpage 固定高预览 + `HtmlFullpageViewer` 全屏查看器，分流判定 `HtmlCardDisplay` 纯函数，终判持久化消息 metadata；零桥接不变；远程资源放行，a 外链点击经 `HtmlLinkPreviewOverlay` 全屏落地页打开，详见 JS_ENGINE_TECH_SPEC.md §7.1） |
...
| 用户问题上报（report-issue） | 设置页「其他」分组「上报问题」行（`features/settings/ReportIssueEntry.kt`，自包含条目）→ `IssueReportClient`（`data/remote/picme/`）→ `POST /v1/report-issue` | 用户问题描述经服务端脱敏后自动在 `littleseven/polang` 创建 GitHub issue；管理后台「问题诊断」页（`/admin/diagnosis`）承载上报列表；2026-09-26 入口自 Chat 顶部栏迁入设置页 |
| 工程师任务卡（TASK_CARD） | `sendClaudeMessage` 提交即插卡 → `EngineerTaskReducer`（SSE 事件→五态迁移 + 审批回填幂等，`features/chat/engineer/`）→ `EngineerTaskCard`（`features/chat/components/`，审批唯一入口；会话含卡时气泡 legacy 继续/交付按钮经 `suppressClaudeActions` 抑制） | 状态存 Room metadata `engineer_task`（REPLACE upsert + `engineerTaskPersistMutex` 串行化 + loadMessages 合并回填 live 优先）；交付 sid 消费端校验 12-hex pattern 回落 VM 级 claudeSid；`/task` DEBUG 冒烟入口注入五态样本（`EngineerTaskSmokeSamples`）；错误信息一等展示（2026-09-26）：`EngineerTaskErrorBlock`（错误色容器+图标+6 行）覆盖 FAILED/AWAITING_DELIVER/已裁决 COMPLETED 三态（errorSummary 存在即展示），AWAITING_CONTINUE 补 `truncatedReason` 行，meta 行补 `deliverBranch`（⎇ 前缀）；任务中心列表项复用同款区块（maxLines=3）；spec `docs/superpowers/specs/2026-09-25-engineer-task-card-design.md` |
| 任务中心（US-12~16，2026-09-26 提前落地） | Chat 顶栏任务图标（`activeEngineerTaskCount` 角标）/ 任务卡「查看全部」→ `task_center` 路由 → `TaskCenterScreen`（双 Tab「工程师任务 \| 后台任务」，Tab 标题带活跃计数徽标分解顶栏角标总数；进行中/历史分区 + 审批动作）→ 点击项经 Activity 级 `chatTaskAnchor` 回 chat 锚定 | 数据源 = `ChatMessageDao.getTaskCardMessages()`（跨会话 `type='task_card'` 查询，TEXT 列零迁移）+ `ChatSessionDao.getAllSessions()` join 会话标题；分区纯逻辑 `TaskCenterPartition`（JVM 单测）；实时刷新靠 Room Flow（结构性事件逐条落库，无新通道）；审批动作复用共享 ChatViewModel 的 `deliverEngineerTaskFromCenter` 等入口（`primeEngineerTask` Room 补种防跨会话空转；deliver/skip/abandon 页内完成，continue/retry 切会话后回 chat）；活跃标记与角标同口径：工程师活跃项 primary chip（待审批加描边）、用户任务卡活跃态（PENDING/RUNNING/PAUSED）图标与状态字主色（`UserTaskCard`）；回联（US-4~6）仍属 P3，依赖 P2 网关改造 |
