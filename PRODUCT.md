...
> **版本**：3.1（破浪相册）· 应用版本 v1.0.39 (10039)  
> **状态**：生效中  
**最后更新**：2026-09-26
**维护者**：项目开发者
...
> **2026-09-26 产品决策：任务范式升格为一等公民（参照 Meta Muse 调研）**：相册管理的演进方向由「Chat 式相册管理与搜索」升格为「**对话式相册管理（自然语言委托 → 后台任务执行 → 完成回联）**」，与「图片后处理智能编辑」并列为双支柱。「任务」成为与对话、编辑并列的显性产品概念——相册域长耗时操作（打标扫描 / 去重扫描 / 整理清理 / 模型下载）在产品语义上升格为「用户委托 Agent 执行的任务」，统一在任务中心观察与控制。底座已落地（任务中心双 Tab + `UserTask` 协议 M1（TAG 扫描 + 模型下载收编）+ 工程师任务卡，`feat/task-center` 已合 main）。差异化叙事锚定**「隐私版 Agent 任务」**——同样的后台任务范式，任务数据 100% 不出端（Muse 跑在每用户云端 VM，PoLang 媒体任务全端侧，ADR-008；Muse Image 三天下架事件反向验证该路线）。路线图新增横切「任务范式线」（§6.6），Muse Top5 借鉴项全量落位；spec：`docs/superpowers/specs/2026-09-26-task-paradigm-positioning-design.md`。
...
**当前聚焦**：演进方向为「对话式相册管理（自然语言委托 → 后台任务执行 → 完成回联）+ 图片后处理智能编辑」（2026-09-26 任务范式升格，见头部决策注记）。从「相机工具」向「AI 相册助手」转型，以相册首页为核心入口，底部悬浮 Tab 聚合 相册/整理/聊天/人物/回忆 五项（2026-09-06 导航统一，与主页面 Pager 页序 1:1；打标已并入整理页 SCAN tab），相机为路由化全屏页（仅头像拍摄进入），模型中心入口位于相册顶栏与设置主菜单，设置入口位于顶部栏最右侧，相册/编辑作为核心能力被 Agent 调度。
...
**任务层（✅ 底座已落地 · 2026-09-26 升格为一等产品概念）**
- **定位**：承载「自然语言委托 → 后台任务执行 → 完成回联」任务范式——相册域长耗时操作（打标扫描 / 去重扫描 / 整理清理 / 模型下载）与工程师代码任务统一以「任务」呈现：用户委托、后台执行、任务中心观察与控制（回联三件套规划中，§6.6）
- **任务中心（✅ 已落地）**：「app 正在替我干的事」的统一观察与控制面——Chat 顶栏任务图标入口（角标 = 工程师进行中数 + 用户任务进行中数合并计数）；双 Tab「工程师任务 | 后台任务」；进行中 / 历史双分区（历史封顶 50 条）；点击任务卡跳转来源场景（详见 FEATURES.md §3.6）
- **用户任务协议（✅ M1 已落地）**：`UserTask` 统一模型——6 态生命周期（PENDING/RUNNING/PAUSED/COMPLETED/FAILED/CANCELLED）+ 声明式动词（暂停/恢复/取消/重试，按状态推导）；身份落 Room（`user_task` 表，AppDatabase v24）重启可见 + 高频进度走内存流；启动对账（进程死于任务中 → FAILED+重试 可见）；已接入 TAG 扫描 + 模型下载，去重扫描 / 美学打分 / 人脸重聚类待收编（M2/M3，§6.6）
- **工程师任务卡（✅ 已落地）**：对话流 `TASK_CARD` 消息——进度 / 审批 / 终态三形态，审批唯一入口（截断续跑、交付授权收口到卡片，详见 FEATURES.md §2.7）
- **语义承诺**：出现在任务中心的任务都可控——未接入可控协议的体系不出现（可控性协议化，而非只读聚合）
- **差异化**：「隐私版 Agent 任务」——同样的后台任务范式，任务数据 100% 不出端（ADR-008；对照 Muse 云端 VM）
...
| **任务中心（双 Tab）** | ✅ | Chat 顶栏入口 + 角标合并计数；工程师任务 \| 后台任务双 Tab，进行中/历史分区（`feat/task-center` 已合 main，详见 FEATURES.md §3.6） |
| **用户任务协议 M1** | ✅ | `UserTask` 统一生命周期 + 声明式动词 + 启动对账；TAG 扫描 / 模型下载两适配器（`user_task` Room v24） |
| **工程师任务卡** | ✅ | `TASK_CARD` 进度/审批/终态，审批唯一入口；回联三件套（US-4~6）待 P2 网关改造（§6.6） |
...
PoLang 以技术探索与能力验证为核心目标，**聚焦 Gallery/Editor + AI Chat 两条核心线 + 任务范式横切线**（演进方向 2026-09-26 升格为：**对话式相册管理（自然语言委托 → 后台任务执行 → 完成回联）+ 图片后处理智能编辑**；2026-08-16 原表述「Chat 式相册管理与搜索 + 图片后处理智能编辑」），其余产品线冻结至 Phase 3+ 按需解冻：
...
| **任务范式** | **横切范式线（一等公民基础设施）** | 对话委托 → 后台任务执行 → 完成回联；隐私版 Agent 任务（数据不出端） | Agent 任务范式的端侧工程实践 | 持续投入（§6.6） |
...
相册+编辑（核心产品与默认首页）+ AI 对话（核心助手能力）→ 双核并行推进
    ↓
        任务范式（横切）统一承载：任务中心 + UserTask 协议 + 任务卡（委托 → 后台执行 → 完成回联）
...
### 6.6 任务范式线（Task Paradigm）— 横切范式线（2026-09-26 升格，参照 Meta Muse 调研）

> **定位**：任务是一等公民。承载「对话式相册管理（自然语言委托 → 后台任务执行 → 完成回联）」中的任务范式本体，横切服务相册及编辑线与 AI 对话线（用户任务承接相册域长耗时操作，工程师任务承接对话内代码任务）。差异化锚定「隐私版 Agent 任务」：同样的后台任务范式，任务数据 100% 不出端（对照 Muse 云端 VM）。
> **SSOT**：`docs/superpowers/specs/2026-09-26-user-task-protocol-design.md`（用户任务协议 + M1/M2/M3 分期）、`docs/superpowers/specs/2026-09-25-engineer-task-card-design.md`（工程师任务卡 + P1/P2/P3 分期）；本节为产品路线视角。

| 功能 | 状态 | 优先级 | 说明 |
|------|------|--------|------|
| 任务中心（双 Tab + 角标合并） | ✅ 已落地 | P0 | 「工程师任务 \| 后台任务」双 Tab；进行中/历史分区（历史封顶 50）；Chat 顶栏入口 + 角标合并计数 |
| 用户任务协议 M1 | ✅ 已落地 | P0 | `UserTask` 统一模型 + `UserTaskRegistry`（Room v24）+ 启动对账；TAG 扫描 + 模型下载两适配器收编 |
| 工程师任务卡 | ✅ 已落地 | P0 | `TASK_CARD` 进度/审批/终态，审批唯一入口；任务中心页已顺车落地（US-12~16） |
| **审批三要素卡** | 📋 规划中 | P0 | DESTRUCTIVE 批量操作前聚合确认卡：「将删除 N 张 · 含 X 张收藏 · 回收站可恢复」（Muse 借鉴 #2，强化既有 CommandRisk / Tier A 体系，无前置依赖） |
| **回联三件套** | 📋 规划中 | P1 | 断连对账 / 本地通知 / 进程重建恢复（工程师任务 US-4~6）；**硬依赖 P2 网关改造**（回合与 SSE 解耦、per-sid 状态落盘、sid↔owner 校验） |
| **Agent 审计日志** | 📋 规划中 | P1 | `agent_actions` 结构化审计（命令/风险级/结果/时间）+ 设置页查看（Muse 借鉴 #3，随回联顺带） |
| 用户任务 M2 收编 | 📋 规划中 | P2 | 去重扫描（先补 FGS / 进程外生命周期，再写适配器） |
| 用户任务 M3 收编 | 📋 规划中 | P2 | 美学打分、人脸重聚类（补动词与进度通道） |
| **每周相册简报 feed** | 📋 规划中 | P1 | WorkManager 定时 + JS 统计 + 模板卡渲染（Muse 借鉴 #4；**依赖 ADR-014 HTML 卡落地**） |
| 记忆主动化 | 📋 规划中 | P2 | 基于事实记忆/人物关系的主动建议（「去年今日」；Muse 借鉴 #5 验证方向；维持 §5.2 既有 P2 规划） |

**演进路线**：
- **已落地（M1）✅**：任务中心双 Tab + 用户任务协议（TAG 扫描/模型下载）+ 工程师任务卡与任务中心页
- **近期**：审批三要素卡（无前置，性价比最高）；回联三件套（随 P2 网关改造）；审计日志顺带
- **中期**：M2/M3 收编（去重 → 美学 → 重聚类）；周报 feed（随 ADR-014）
- **明确不借**（Muse 调研 §3.3 存档）：跨 App 执行/支付、Custom Connector（LLM 产代码，违背 ADR-014 D5 铁律）、实时 Avatar、云 VM——与业务边界/产品决策相斥
